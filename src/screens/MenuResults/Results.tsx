...
const randomColor = (name, lastname) => {
  const colors = [
    [39, 174, 96],
    [45, 156, 219],
    [200, 0, 0],
...
const Results = ({ navigation, route }: ResultsScreenProps) => {
  ...
  let resultados = data?.election.results.results.map((res, key) => {
    return {
      ...
      svg: {
        fill: randomColor(res.candidate.firstName, res.candidate.lastName)
      }
    }
  })
  ...
  const totalVotesToCandidate =
    data?.election.results.votesCast - data?.election.results.whiteVotes
